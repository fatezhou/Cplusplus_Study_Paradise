...
ZoyeeUtils::CDebugView::CDebugView( std::vector<std::string>& vecProcessName )
{
	Init();
...
ZoyeeUtils::CDebugView::CDebugView( std::vector<int>& vecPid )
{
	Init();
...
void ZoyeeUtils::CDebugView::Start( bool bAsyc /*= false*/ )
{
	if (bAsyc){
...
void ZoyeeUtils::CDebugView::SetPrintFunction( pPrintCallback _callback )
{
	if (_callback){
...
ZoyeeUtils::CDebugView::~CDebugView( void )
{

}

void ZoyeeUtils::CDebugView::AddProcess( const char* pProcessName )
{
#ifdef USE_AUTO_LOCK
	_LOCK_;
#endif
...
int ZoyeeUtils::CDebugView::GetProcessId( char* pProcessName )
{
#ifdef USE_AUTO_LOCK
	_LOCK_;
#endif
...
char* ZoyeeUtils::CDebugView::GetProcessName( int nPid )
{
#ifdef USE_AUTO_LOCK
	_LOCK_;
#endif
...
void ZoyeeUtils::CDebugView::SetMapProcess()
{
	PROCESSENTRY32 pe32;
...
void ZoyeeUtils::CDebugView::Init()
{
	SetPrintFunction(nullptr);
	SetMapProcess();
}

void ZoyeeUtils::CDebugView::AddPid( const int nPid )
{
#ifdef USE_AUTO_LOCK
	_LOCK_;
#endif
...
void ZoyeeUtils::CDebugView::DefaultPrint( const char* pProcessName, const int nPid, char* pBuff )
{
	printf("[%d][%s]: %s\n", nPid, pProcessName, pBuff);
}

int ZoyeeUtils::CDebugView::Listen(void* pThis)
{
